...
5. **Knowledge Graph Traversal (GraphRAG):**
...
# --- PHASE 2: Intelligent Follow-up Detection & Query Refiner ---
query_refiner_agent = create_agent(
    llm,
    [],
    system_message="""You are an intelligent context manager and query refinement agent. 
Analyze the user's latest query in the context of the conversation history. 
If the new query is a follow-up (e.g., uses pronouns like "it", "they", or refers to previous topics), rewrite it into a fully standalone query by resolving those references using the conversation history.
Also, fix any spelling mistakes or expand abbreviations to make it clear for a search system.
If the query is already standalone and clear, output it as is (with corrections if needed).
Do not answer the question.
Output exactly: 'REFINED QUERY: ' followed by the refined/standalone query."""
)

query_refiner_node = functools.partial(
    agent_node, agent=query_refiner_agent, name="QueryRefiner"
)
# --------------------------------------------------------------
